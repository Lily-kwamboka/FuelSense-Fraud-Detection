...
export default function ATGConfig({ api }) {
    ...
    async function handleTest() {
        ...
        setTestResult(null);
        try {
            const res = await fetch(`${api}/api/admin/atg-config/${selectedStation}/test`, {
                method: 'POST',
            });
            const data = await res.json();
            setTestResult(data);
        ...
        <div>
            ...
            {config && !loading && (
                <>
                    ...
                    <div style={sectionStyle}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '12px' }}>
                            <div style={{ gridColumn: 'span 1' }}>
                                ...
                                <input
                                    ...
                                    onChange={e => setConfig({ ...config, gateway_ip: e.target.value })}
                                    placeholder="e.g. 196.216.10.45"
                                    style={inputStyle}
                                />
                                <div style={{ fontSize: '11px', color: '#aaa', marginTop: '4px' }}>
                                    The local or public IP of the ATG console or cellular gateway
                                </div>
                            </div>
                            <div>
                                ...
                                <input
                                    type="number"
                                    value={config.gateway_port || 10001}
                                    onChange={e => setConfig({ ...config, gateway_port: parseInt(e.target.value) })}
                                    placeholder="10001"
                                    style={inputStyle}
                                />
                                <div style={{ fontSize: '11px', color: '#aaa', marginTop: '4px' }}>
                                    Default is 10001 for Veeder-Root TLS
                                </div>
                            </div>
                            <div>
                                ...
                                <input
                                    type="number"
                                    value={config.connection_timeout_ms || 5000}
                                    onChange={e => setConfig({ ...config, connection_timeout_ms: parseInt(e.target.value) })}
                                    placeholder="5000"
                                    style={inputStyle}
                                />
                                <div style={{ fontSize: '11px', color: '#aaa', marginTop: '4px' }}>
                                    How long to wait before declaring the connection failed
                                </div>
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                            <div>
                                ...
                                <select
                                    value={config.console_type || 'veeder_root_tls'}
                                    onChange={e => setConfig({ ...config, console_type: e.target.value })}
                                    style={inputStyle}
                                >
                                    {CONSOLE_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                                </select>
                                <div style={{ fontSize: '11px', color: '#aaa', marginTop: '4px' }}>
                                    Determines which protocol FuelSense uses to communicate with the hardware
                                </div>
                            </div>
                        ...
                        {(config.last_connected_at || config.last_error) && (
                            <div style={{ background: '#f8f8f8', borderRadius: '8px', padding: '12px 14px', fontSize: '12px' }}>
                                {config.last_connected_at && (
                                    <div style={{ color: '#1e8449', marginBottom: '4px' }}>
                                ...
                                {tanks.map(tank => (
                                    <div key={tank.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: '#f8f8f8', borderRadius: '8px', border: '1px solid #e0e0e0' }}>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                            ...
                                            {tank.atg_probe_id ? (
                                                <span style={{ background: '#eafaf1', color: '#1e8449', padding: '4px 10px', borderRadius: '6px', fontSize: '12px', fontWeight: '600', fontFamily: 'monospace' }}>
                                                    {tank.atg_probe_id}
                                                </span>
                                            ) : (
                                                <span style={{ background: '#fdecea', color: '#e74c3c', padding: '4px 10px', borderRadius: '6px', fontSize: '12px', fontWeight: '600' }}>
                                                    ⚠ Not set — go to Tanks page
                                                </span>
                                            )}
                                        </div>
                                    </div>
                    ...
                    <div style={sectionStyle}>
                        ...
                        <button
                            onClick={handleTest}
                            disabled={testing || !config.gateway_ip}
                            style={{ padding: '9px 20px', background: testing || !config.gateway_ip ? '#ccc' : '#1a5276', color: '#fff', border: 'none', borderRadius: '8px', cursor: testing || !config.gateway_ip ? 'not-allowed' : 'pointer', fontSize: '13px', fontWeight: '600' }}
                        >
            ...
            {!selectedStation && !loading && (
                <div style={{ background: '#fff', borderRadius: '12px', padding: '60px 24px', textAlign: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                    ...
                    <div style={{ fontSize: '13px', color: '#888' }}>Select a station above to configure its ATG hardware connection.</div>
                </div>
            )}
        </div>
    );
