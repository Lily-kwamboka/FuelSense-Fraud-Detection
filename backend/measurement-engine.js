...
async function lookupStrappingTable(tankId, depthMm) {
    ...
    if (!floorRow) {
        throw new Error('Strapping table lookup failed: depth ' + floorMm + 'mm not found for tank ' + tankId);
    }
...
if (require.main === module) {
    (async () => {
        ...
        assert('VCF = 1.0 at exactly 15C (petrol, density 0.740)',  calculateVCF(15.0, 0.740, 'petrol'),  1.0,   0.000001);
        assert('VCF ~= 0.989 at 25C (petrol, density 0.740)',       calculateVCF(25.0, 0.740, 'petrol'),  0.989, 0.001);
        assert('VCF = 1.0 at exactly 15C (diesel, density 0.835)',  calculateVCF(15.0, 0.835, 'diesel'),  1.0,   0.000001);
        assert('VCF > 1.0 when temp < 15C', calculateVCF(5.0, 0.740, 'petrol') > 1.0 ? 1 : 0, 1, 0);
        ...
        try {
            ...
            assert('Volume at 1000mm ~= 7854L',         vol1000, 7853.982,  1.0);
            assert('Volume at 2000mm ~= 15708L',        vol2000, 15707.963, 1.0);
            assert('Linear interpolation at 500.5mm',   vol500_5, (vol500 + vol501) / 2, 0.01);
